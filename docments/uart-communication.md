# Portenta H7 To Nicla Vision UART Communication

## Objective

The goal of this test is to send processed Nicla Vision camera results directly to the Portenta H7 over a four-wire UART and power connection. The Nicla Vision performs the frame capture and brightness summary. The Portenta H7 receives the result lines and exposes them to the PC over USB serial.
...
| Direction verified | Nicla Vision to Portenta H7 |
...
The final retest passed with the Nicla USB cable disconnected and Portenta USB connected. The Portenta receiver printed continuous `UART_RX` JSON lines from the Nicla Vision camera sender. The 60-second test reached receive counters above `lines=88` and `bytes=10677`, confirming sustained one-way UART communication.
...
- The current payload is a bring-up result, not a final perception algorithm.
- JSON over UART is easy to inspect during bring-up. If throughput becomes important, the same wiring can carry a smaller binary packet later.
